...
| **Side epics** (13 islands, 26 lessons, 58 questions) | Two more journeys from scratch, each with its own map, unlock chain, relics and trials. **The Argonautica (Ceph)**, 7 islands: what storage is → Ceph's daemons → deploying with cephadm → pools, PGs and CRUSH → Ceph as OpenStack's storage → operations → troubleshooting and design. **The Labours of the Helmsman (Kubernetes)**, 6 islands: containers → pods and Deployments → Services and Octavia load balancers → Cinder CSI storage → clusters as a service with Magnum and Cluster API → security, upgrades and troubleshooting. Beginner "In plain words" boxes and "By the end of this island you can…" goals throughout. |
| **The Oracle** (15 scenarios) | Branching troubleshooting trials based on real incidents: No valid host, DHCP failures, floating IPs, MTU, Fernet keys, stuck volumes, RabbitMQ partitions, live migration, Octavia, noisy neighbours. |
| **Terminal labs** (8 labs) | A stateful `openstack`, `ceph` and `rbd` CLI simulator (including a failed-disk drill) in the browser. It produces real-looking tables and real error messages, and supports `-f json/value`, `-c`, tab completion and command history. Objectives are checked automatically. |
